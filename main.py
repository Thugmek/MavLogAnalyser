...
parser.add_argument("log", metavar="LOG", nargs='?', default="/home/thugmek/Downloads/00000007.BIN")
...
def estimate(filename):
    ...
    last_point_decimated = None
    total_dist_decimated = 0
    dist_incremental_decimated = []
    decimation_counter = 0

    while True:
        # we want replay sensor data, plus EKF3 result and SITL data
        m = mlog.recv_match(
            type=['XKF1', 'SIM', 'RFRH', 'RFRF', 'RISH', 'RISI', 'RGPH', 'RGPI', 'RGPJ', 'RFRH', 'RBRH', 'RBRI', 'RMGH',
                  'RMGI', 'GPS'])
        if m is None:
            break
        t = m.get_type()
        ...
        if t == 'GPS':
            curr_point = (m.Lat, m.Lng)
            if last_point:
                dist = geopy.distance.geodesic(last_point, curr_point).m
                total_dist += dist
                print(f"dist: {dist}, total: {total_dist}")
                dist_incremental.append(total_dist)
            last_point = curr_point

            if decimation_counter == 0:
                if last_point_decimated:
                    dist = geopy.distance.geodesic(last_point_decimated, curr_point).m
                    total_dist_decimated += dist
                    print(f"decimated: dist: {dist}, total: {dist_incremental_decimated}")
                    dist_incremental_decimated.append(total_dist_decimated)
                decimation_counter = 10
                last_point_decimated = curr_point
            else:
                decimation_counter -= 1

    import matplotlib.pyplot as plt
    plt.plot(dist_incremental)
    plt.plot(dist_incremental_decimated)
    plt.show()
